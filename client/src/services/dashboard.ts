import { Article, ItemsByType, KeyMetric, MaterialType, TraceActions } from '@/types/dashboard'
...
export const getTraceActions = async () => {
  const response = await api.get<TraceActions>('/trace_actions')
  return response?.data
}
